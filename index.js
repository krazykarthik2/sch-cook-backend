...
app.get("/branch/get/simple", async (req, res) => {
  try {
...
//curl -X GET http://localhost:5000/branch/get/simple
...
app.get("/branchcode/get", async (req, res) => {
  try {
    const branchcodes = await getBranchCodes(req.query);
...
app.get("/relation/get", async (req, res) => {
  try {
    const relations = await getEmpRelations(req.params);
    res.json(relations);
  } catch (error) {
    res.status(500).send(error.message);
  }
